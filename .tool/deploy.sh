...
#   DEPLOY_SCRIPT  远程部署脚本路径 (默认: /opt/trainlobster/deploy.sh)

set -e

REMOTE_HOST="${REMOTE_HOST:-root@47.93.214.178}"
REMOTE_SCRIPT="/opt/trainlobster/deploy.sh"
VERBOSE=false
...
ssh "$REMOTE_HOST" "cd /opt/trainlobster && bash $REMOTE_SCRIPT"

if [ "$VERBOSE" = true ]; then
  echo ""
  echo "✅ 部署完成！"
  echo ""
  echo "🌐 访问地址: https://trainlobster.com"
fi
